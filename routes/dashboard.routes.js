...
router.get("/api/dashboard/recruiter/:uid", async (req, res) => {
  try {
    ...
    // 2. Fetch jobs posted by this recruiter (multiple ways to match)
    const jobs = await db
      .collection("find_jobs")
      .find({
        $or: [
          { recruiterUid: uid }, // Direct UID match
          { recruiterEmail: user.email }, // Email match
          { company: user.companyName }, // Company match
          { company: user.displayName }, // Display name fallback
        ],
      })
      .sort({ createdAt: -1 })
      .toArray();
    ...
    applications.forEach((app) => {
      ...
      applicantsCountByJob[jobId] = (applicantsCountByJob[jobId] || 0) + 1;
    });

    // Add applicant counts to jobs
    const jobsWithCounts = jobs.map((job) => ({
      ...job,
      applicantsCount: applicantsCountByJob[job._id.toString()] || 0,
    }));
    ...
    // 5. Top Candidates with Communication Scores
    ...
    res.status(200).json({
      ...
      data: {
        stats,
        jobs: jobsWithCounts.slice(0, 5), // Latest 5 jobs with counts
